...
const checkForSdkUpdates = async () => {
  try {
    const {
      response
    } = await _RESTController.default.ajax('GET', 'https://registry.npmjs.org/-/v1/search?text=moralis&size=1');
    const latestVersion = response.objects[0].package.version;
    if ("0.0.140" < latestVersion) // eslint-disable-next-line no-console
